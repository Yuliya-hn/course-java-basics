...
public class Task04 {
    public static void main(String[] args) {
        ...
        double totalHP = healthPoints;
        double hours = 1;

        while ( hours <= hoursAfterRespawn ){
            double percent = totalHP * (regenerationPercentFromCurrentHealth / 100);
            totalHP += percent;
            hours ++;
        }

        System.out.println( (int) Math.floor(totalHP));
    }
}